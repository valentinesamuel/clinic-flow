...
const successHandler = async (response: AxiosResponse): Promise<AxiosResponse> =>
...
const errorHandler = (error: AxiosError): Promise<AxiosError> => {
  if (error.response) {
    ...
    if (typeof errorData.error !== 'string') {
      message = `${errorData.message} - ${errorData.error?.[0]?.errors?.[0] ?? ''}`.trim();
    }

    const errorType = typeof errorData.error === 'string' ? errorData.error : 'Form error';
